...
const StaffLayout = ({ children }) => {
  ...
  // Data Navigasi Staff (Sesuai Input Anda)
  const navigation = [
    { name: 'Dashboard', href: '/staff', icon: Home },
    { name: 'Jadwal Acara', href: '/staff/jadwal-acara', icon: SplitSquareHorizontalIcon },
    { name: 'Dokumentasi', href: '/staff/dokumentasi', icon: Camera },
    { name: 'Papan Disposisi', href: '/staff/leaderboard', icon: TrendingUpIcon },
    { name: 'Daftar User', href: '/staff/daftar-user', icon: UserCircle2 }
  ];

  const isActive = (path) => location.pathname === path;

  // --- REUSABLE COMPONENT: BRAND LOGO ---
  const BrandLogo = ({ className = "" }) => (
    <div className={`flex items-center gap-3 ${className}`}>
      {/* Mengubah brightness menjadi grayscale agar sesuai tema gelap */}
      <img src={img} alt="Magessa Logo" className='h-6 object-contain grayscale-100' />
      <div className='flex flex-col'>
        <h1 className='text-lg font-medium bg-gradient-to-r from-white to-neutral-400 bg-clip-text tracking-tight text-transparent py-2 leading-none'>
          Magessa
        </h1>
      </div>
    </div>
    ...
    <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-white/20 overflow-x-hidden">
        ...
        <div className="w-64 flex justify-end">
          <button className='flex items-center gap-3 pl-1 pr-2 py-1 rounded-full hover:bg-white/5 border border-transparent hover:border-white/5 transition-all group'>
            <div className="text-right hidden xl:block mr-1">
              <p className="text-sm font-medium text-white leading-none">Staff</p>
              <p className="text-[10px] text-gray-500 mt-1">General Access</p>
            </div>
            {/* Avatar dengan Soft Glow */}
      ...
      <div className='flex pt-20 min-h-screen'>
        ...
        <aside
          ...
          style={{
            width: sidebarHovered ? '260px' : '88px',
